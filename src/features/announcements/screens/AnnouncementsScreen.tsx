...
function AnnouncementCard({ item, isAdmin, onEdit }: { item: Announcement; isAdmin: boolean; onEdit?: (item: Announcement) => void }) {
...
export function AnnouncementsScreen({ onAdd, onBack, onEdit }: AnnouncementsScreenProps) {
  const { theme } = useTheme();
    ...
    <ScreenWrapper scrollable={false} padded={false}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <AnnouncementCard item={item} isAdmin={isAdmin} onEdit={onEdit} />}
        />
      )}
    </ScreenWrapper>
  );
...
const styles = StyleSheet.create({
  ...
  adminActions: { flexDirection: 'row', gap: spacing[3] },
